 ...
 *     <li>transformation subqueries into top-level queries</li>
 ...
 * that should hold for {@link tools.vitruv.neojoin.validation valid} queries. Invariants that should always hold are
 * checked with assertion methods from {@link tools.vitruv.neojoin.utils.Assertions Assertions}.
 */
public class AQRBuilder {
	 ...
	 */
	public AQR build() {
		invariant(targetClasses.isEmpty(), "AQR already built");
	...
	private void populateTargetClass(AQRTargetClass result, @Nullable Body body) {
		if (body != null) { // create features based on definition in Body
			for (Feature feature : body.getFeatures()) {
				result.features().add(createFeature(feature));
			}
		} else { // copy all features from the source class
			invariant(result.source() != null, "Query without source must have a body");
			invariant(result.source().joins().isEmpty(), "Query with join must have a body");

			var sourceClazz = result.source().from().clazz();
			for (var feature : sourceClazz.getEStructuralFeatures()) {
				result.features().add(copyFeature(feature));
			}
		}
	}
